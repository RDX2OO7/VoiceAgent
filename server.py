...
logger = logging.getLogger("voice-agent.production")

APP_NAME = "adk-voice-agent-production"
...
app = FastAPI(title="ADK Voice Production Agent")
...
async def voice_socket(websocket: WebSocket) -> None:
    ...
    if origin and ALLOWED_ORIGINS and "*" not in ALLOWED_ORIGINS:
        is_local = origin.startswith("http://127.0.0.1:") or origin.startswith("http://localhost:") or origin.startswith("https://127.0.0.1:") or origin.startswith("https://localhost:")
        if origin not in ALLOWED_ORIGINS and not is_local:
